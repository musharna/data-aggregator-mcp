...
- **DataONE object fetch** `https://cn.dataone.org/cn/v2/resolve/{pid}` (or `/object/{pid}`) streams the object bytes.
...
- `resolve(client, resource_id)`: fetch the metadata Solr doc by `q=identifier:"<pid>"`. Then list its data objects (Solr `q=resourceMap:"<rmap>"` filtered to `formatType:DATA`, or the doc's `documents` field) and build `files[]`: `FileEntry(name=<filename or pid>, url=f"https://cn.dataone.org/cn/v2/resolve/{data_pid}", checksum=f"{algo.lower()}:{checksum}" if checksum else None, size=<size>)`. Raise `NotFoundError` on empty.
